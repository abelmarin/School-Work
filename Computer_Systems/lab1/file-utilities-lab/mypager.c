...
int fetch_next_line(char line[]) {
	int current = 0;
	current = fetch_next_word(line, 81);
	return current;
}
...
int fetch_next_word(char word[], int max_size) {
	...
	while (sizeOfWord < max_size) {
        if (buffer[0] != '\n'){
            word[sizeOfWord] = buffer[0];
            currentBuff = refill_buffer(0);
            sizeOfWord++;
            if (currentBuff == 0) 
                return 0;
        } else {
...
int refill_buffer(int start) {
	int bytesBeingRead;
	bytesBeingRead = read(fd, buffer+start, 1);
	if(bytesBeingRead == 0) {
		return 0;
	} else {
