...
-- (For study and review purposes only. Copying of any part of this document 
-- into a submitted assignment constitutes plagiarism.)
...
select *
    from student
    where city = 'Springfield' and started between 2011 and 2013;
...
select *
    from student
    where started = (select max(started) from student);

select firstname, lastname
    from student
    where sid in (select studentid
                  from membership
                  where groupid = (select gid
                                    from studentgroup
                                    where name = 'DeFrag'));
