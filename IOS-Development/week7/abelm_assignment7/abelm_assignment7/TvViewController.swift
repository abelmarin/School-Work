...
class ChannelList {
    ...
    var channelNumbers: [String] = []
    
    init() {}
    init (labels: [String], numbers: [String]) {
        if labels.count == 4 && numbers.count == 4 {
            self.channelNames = labels
            self.channelNumbers = numbers
        }
    }
    
    func set(label: String, channel: String, arrayPosition: Int) {
        channelNames[arrayPosition] = label
        channelNumbers[arrayPosition] = channel
    }
    
}

var currentChannelList = ChannelList(labels: ["ABC", "NBC", "CBS", "FOX"], numbers: ["07", "08", "09", "10"])
...
class TvViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        favoriteSegmentedControl.setTitle(currentChannelList.channelNames[0], forSegmentAt: 0)
        favoriteSegmentedControl.setTitle(currentChannelList.channelNames[1], forSegmentAt: 1)
        favoriteSegmentedControl.setTitle(currentChannelList.channelNames[2], forSegmentAt: 2)
        favoriteSegmentedControl.setTitle(currentChannelList.channelNames[3], forSegmentAt: 3)
    ...
    @IBAction func favoriteSelected(_ sender: UISegmentedControl) {
        if let name = sender.titleForSegment(at: sender.selectedSegmentIndex) {
            if name == currentChannelList.channelNames[0] {
                channelLabel.text = currentChannelList.channelNumbers[0]
            } else if name == currentChannelList.channelNames[1] {
                channelLabel.text = currentChannelList.channelNumbers[1]
            } else if name == currentChannelList.channelNames[2] {
                channelLabel.text = currentChannelList.channelNumbers[2]
            } else if name == currentChannelList.channelNames[3] {
                channelLabel.text = currentChannelList.channelNumbers[3]
            }
        }
    }
