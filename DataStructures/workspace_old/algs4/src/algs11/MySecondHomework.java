...
public class MySecondHomework {
	...
	public static double sum (double[] a) {
		double result = sumHelper(a,0);
		return result;
	}
	private static double sumHelper (double[] a, int i) {
		double result = 0;
		if (i < a.length) {
			result = sumHelper(a,i+1);
			result += a[i];
		}
		return result;
	}
	...
	public static void reverse (double[] a) {
		reverseHelp(a, a.length-1, 0);
	}
	private static void reverseHelp (double[] a, int hi, int lo) {
		if (lo<hi) {
			double loVal = a[lo];
			double hiVal = a[hi];
			a[hi] = loVal;
			a[lo] = hiVal;
			reverseHelp(a,hi-1,lo+1);
		}
	}
	...
	public static void draw (double centerX, double centerY, double radius) {
		...
		draw(centerX+change, centerY+change, radius*.5);
		draw(centerX-change, centerY+change, radius*.5);
	}
