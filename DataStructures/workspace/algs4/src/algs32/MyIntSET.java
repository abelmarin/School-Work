...
/* ************************************************************************
 *
 *    OUTPUT ABEL MARIN: Finished Tests
 *
 *************************************************************************/
...
public class MyIntSET {
	...
	private int sizeHelper(Node bst) {
		if (bst == null) return 0; 
        return sizeHelper(bst.left) + 1 + sizeHelper(bst.right); 
	}
	...
	private int heightHelper(Node bst) {
		if (bst == null) return -1;
    	int height = Math.max(heightHelper(bst.left), heightHelper(bst.right)) + 1;
		return height;
	}
	...
	private int oddHelp(Node bst) {
		if (bst == null) return 0;
		else return oddHelp(bst.left) + oddHelp(bst.right) + (bst.key % 2);
	}
	...
	public int sizeAtDepth(int k) {
		return atDepthHelper(root, 0, k);
	}	
	private int atDepthHelper(Node bst, int count, int j) {
		if (bst == null) return 0; 
        if (j == 0) count++;
        else {
        	j--;
        	return atDepthHelper(bst.left, count, j) + atDepthHelper(bst.right, count, j); 
        }
        return count;
	}
	...
	public int sizeAboveDepth(int k) {
		return aboveDepthHelp(root, 0, k);
	}
	private int aboveDepthHelp(Node bst, int j, int k) {
		if (bst == null) return 0; 
        if (j < k) {
        	j++;
        	return aboveDepthHelp(bst.left, j, k) + 1 + aboveDepthHelp(bst.right, j, k);
        } 
        else return 0;
	}
	...
	public int sizeBelowDepth(int k) {
		return belowDepthHelp(root, 0, k);
	}
	private int belowDepthHelp(Node bst, int j, int k) {
		if (bst == null) return 0;
        if (j > k) {
        	j++;
        	return belowDepthHelp(bst.left, j, k) + 1 + belowDepthHelp(bst.right, j, k);
        } else {
        	j++;
        	return belowDepthHelp(bst.left, j, k) + belowDepthHelp(bst.right, j, k);
        }
	}
	...
	public boolean isPerfectlyBalancedS() {
		if (perfectSHelp(root) == -1) return false;
		return true;
	}
	private int perfectSHelp(Node bst) {
		if (bst == null) return 0;
		int left = perfectSHelp(bst.left);
		int right = perfectSHelp(bst.right);
	...
	public boolean isPerfectlyBalancedH() {
		if (perfectHHelper(root) == -1) return false;
		return true;
	}
	private int perfectHHelper(Node bst) {
		if (bst == null) return 0;
		int left = perfectHHelper(bst.left);
	    if (left == -1) return -1;

    	int right = perfectHHelper(bst.right);
        if (right == -1) return -1;
	...
	public boolean isOddBalanced() {
		if (oddBalanceHelp(root) == -1) return false;
		return true;
	}
	private int oddBalanceHelp(Node bst) {
		if (bst == null) return 0;
		int left = oddBalanceHelp(bst.left);
		int right = oddBalanceHelp(bst.right);
		if (left == right) return left + right + (bst.key % 2);
		else return -1;
	}
	...
	public boolean isSemiBalanced() {
		if (semiHelp(root) == -1) return false;
		return true;
	}
	private int semiHelp(Node bst) {
		if (bst == null) return 0;
		int left = semiHelp(bst.left);
		int right = semiHelp(bst.right);
		int	size = left + 1 + right;
		
		if (left-right > 0) {
			if (left <= right*3) return size;
			else return -1;
		}
		else if (right-left > 0) {
			if (right <= left*3) return size;
			else return -1;
		}
		else return size;
	}
	...
	public void removeOddSubtrees () {
		if (root == null) return;
		else if (root.key % 2 == 1) root = null;
		else removeOddHelp(root);
	}
	private Node removeOddHelp(Node node) {
		if (node == null) return node;
		node.left = removeOddHelp(node.left);
        node.right = removeOddHelp(node.right);
		if (node.key % 2 == 1) node = null;
		return node;
	}
	...
	public void removeBelowDepth(int k) {
		belowDepthHelper(root, k);
	}
	private void belowDepthHelper(Node node, int k) {
	    if (node == null) return;
	    if (k == 0) {
	    	node.left = null;
            node.right = null;
	    ...
	    k--;
	    belowDepthHelper(node.left, k);
	    belowDepthHelper(node.right, k);
	}
	...
	private Node zeroHelp(Node node) {
		if (node == null) return node;
		if (node.left == null && node.right != null) node.left = new Node(0);
		else if (node.right == null && node.left != null) node.right = new Node(0);
        node.left = zeroHelp(node.left);
        node.right = zeroHelp(node.right);
		return node;
	}
	...
	public void removeLeaves() {
		...
		else leafDeleteHelp(root);
	}
	private Node leafDeleteHelp(Node node) {
		if (node == null) return null;
	    if (node.left == null && node.right == null) return null;
	    else {
	    	node.left = leafDeleteHelp(node.left);
		    node.right = leafDeleteHelp(node.right);
		    return node;
	    }
	}  
	...
	private Node singleHelp(Node node) { 
        if (node == null) return null;
        node.left = singleHelp(node.left);
        node.right = singleHelp(node.right);
	...
	private static void testOne () {
		...
		set1.printLeftI ();
		MyIntSET set2 = MyIntSET.fromString ("41 21 61 11 31 51 71 10 111");
		set2.printLeftI ();
		//show (set1, "set1");
		//show (set2, "set2");
	}
